...
bin/calico-felix-nodbg: bin/calico-felix-nodbg-$(ARCH)
	ln -f bin/calico-felix-nodbg-$(ARCH) bin/calico-felix-nodbg
...
bin/calico-felix-nodbg-$(ARCH): $(SRC_FILES) vendor/.up-to-date
	@echo Building felix-nodbg for $(ARCH) on $(BUILDARCH)
	mkdir -p bin
	$(DOCKER_RUN) $(LOCAL_BUILD_MOUNTS) $(CALICO_BUILD) \
	   sh -c 'go build -v -i -o $@ -v $(LDFLAGS_NODBG) "$(PACKAGE_NAME)/cmd/calico-felix" && \
		( ldd $@ 2>&1 | grep -q -e "Not a valid dynamic program" \
		-e "not a dynamic executable" || \
		( echo "Error: $@ was not statically linked"; false ) )'
